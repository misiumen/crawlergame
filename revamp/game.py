...
class Game:
    ...
    def set_resolution(self, w: int, h: int, fullscreen: bool | None = None) -> bool:
        ...
        if (w, h) not in [tuple(p) for p in
                          __import__("revamp.config", fromlist=["SUPPORTED_RESOLUTIONS"]).SUPPORTED_RESOLUTIONS]:
    ...
    def _open_settings(self):
        ...
        try:
            ...
            cur_res_idx = 0
        self.settings_state = {
            "row": 0,                 # 0=resolution, 1=fullscreen, 2=apply, 3=back
            "res_idx": cur_res_idx,
    ...
    def _attempt_memetic(self, intent):
        ...
        entry triggers)."""
        from . import memetics, risk_reward
        from . import time_system as ts
        from .utils_compat import roll_d20
        import random
        ...
        from .memetics import BeliefEffect, create_seed, register_seed
        try:
            from .data.memetic_templates import EFFECT_TEMPLATES
        except Exception:
            EFFECT_TEMPLATES = {}
    ...
    def _attempt_use_password(self, intent):
        ...
        from . import knowledge as _kn, time_system as ts
        ch = self.world.character
    ...
    def _attempt_invoke_belief(self, intent):
        ...
        """
        from . import knowledge as _kn, memetics
        from .utils_compat import roll_d20
        room = self.world.current_floor.current_room() if self.world.current_floor else None
        if room is None:
            self.log(t("feedback_no_room", fallback="Nie jesteś nigdzie."), LOG_WARN)
